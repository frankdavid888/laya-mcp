...
- `install.bat`（双击入口）
- `install.ps1`（安装逻辑）
- `laya_mcp_server.py`（MCP 服务）
- `register_agents.py`（助手检测 + 注册）
- `README.md`
- `DESCRIPTION.md`
- `models/`（内置模型）
...
- **自动注册**：Claude Code、Codex、Gemini CLI、Grok、OpenCode
- **检测 + 中文提示手动接入**：OpenClaw、Hermes、Pi
...
- **系统**：Windows 10/11，装了上述任一助手；
- **依赖**：一次性联网下载 Python 依赖（约几百 MB），模型离线；
- **硬件**：无需 GPU。
